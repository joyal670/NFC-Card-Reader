...
class CardProvider : ContentProvider() {
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        ...
        when (sUriMatcher.match(uri)) {
            ...
            CardDBHelper.SINGLE_CARD_URI_INDICATOR -> {
                ...
                count = db.delete(
                    CardsTableColumns.TABLE_NAME, CardsTableColumns._ID + "=" + rowId
    ...
    override fun update(uri: Uri, values: ContentValues?, selection: String?, selectionArgs: Array<String>?): Int {
        ...
        val count: Int = when (sUriMatcher.match(uri)) {
            ...
            CardDBHelper.SINGLE_CARD_URI_INDICATOR -> {
                val rowId = uri.pathSegments[1]
                db.update(
                    CardsTableColumns.TABLE_NAME, values, CardsTableColumns._ID + "=" + rowId
                        + if (!TextUtils.isEmpty(selection)) " AND ($selection)" else "", selectionArgs)
            }
            else -> throw IllegalArgumentException("Unknown URI $uri")
        }
    ...
    companion object {
        @NonNls
        val AUTHORITY = "com.dst.testapp" + ".cardprovider"
