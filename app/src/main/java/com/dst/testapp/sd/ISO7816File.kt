...
data class ISO7816File internal constructor(
    ...
    fun showRawData(selectorStr: String): ListItemInterface {
        ...
        if (binaryData != null)
            recList.add(ListItemRecursive.collapsedValue("Localizer.localizeString(R.string.binary_title_format)", binaryData.toHexDump()))
        if (fci != null)
            recList.add(ListItemRecursive("Localizer.localizeString(R.string.file_fci)", null, ISO7816TLV.infoWithRaw(fci)))
