...
import com.dst.testapp.sd.Subscription
import com.dst.testapp.sd.TransitBalance
...
class CardBalanceFragment : ListFragment() {
    ...
    private inner class BalancesAdapter(context: Context, balances: List<Any>) : ArrayAdapter<Any>(context, 0, balances) {
        ...
        private fun getBalanceView(convertView: View?,
                                   parent: ViewGroup, balance: TransitBalance
        ): View {
            val view = Utils.loadMultiReuse(
                convertView,
                activity?.layoutInflater!!,
            ...
            balanceView.text = balanceCur.maybeObfuscateBalance().formatCurrencyString(true)
    ...
    companion object {
        ...
        internal fun subMergeInfos(sub: Subscription): List<ListItemInterface>? =
            Subscription.mergeInfo(sub)
    }
}
