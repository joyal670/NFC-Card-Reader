...
class SmartRiderTransitData(
    ...
    private val localisedTokenType: String?
        get() = when (mSmartRiderType) {
            SmartRiderType.SMARTRIDER -> when (mTokenType) {
                0x1 -> "Standard"
                0x2 -> "School Student"
                0x4 -> "Tertiary Student"
                0x6 -> "Seniors"
                0x7 -> "Concession"
                0xe -> "Staff"
                0xf -> "Pensioner"
                0x10 -> "Convention Pass"
                else -> null
            }
            else -> null
        }?.let { it }
    ...
    override val cardName: String
        get() = mSmartRiderType.friendlyName

    override val info: List<ListItemInterface>
        get() = listOfNotNull(
            ListItem(
               "Ticket type", mTokenType.toString()
            ),
    ...
    companion object {
        ...
        private val SMARTRIDER_CARD_INFO = CardInfo(
            ...
            name = "SmartRider",
            locationId = "Western Australia, Australia",
        ...
        private val MYWAY_CARD_INFO = CardInfo(
            ...
            name = "MyWay",
            locationId = "Australian Capital Territory, Australia",
            cardType = CardType.MifareClassic,
            region = TransitRegion.AUSTRALIA,
            keysRequired = true
        )
