...
data class MetroTimeZone(val olson: String): Parcelable {
    ...
    companion object : KSerializer<MetroTimeZone> {
        ...
        override val descriptor: SerialDescriptor = buildClassSerialDescriptor("MetroTimeZone") {
            element<String>("olson") // Assuming there's a property named 'olson' in MetroTimeZone
        }
...
class EpochUTC internal constructor(private val baseMillis: Long,
                                    ...
                                    private val outputTz: MetroTimeZone
) : Epoch {
    fun mins(offset: Int) =
...
data class TimestampFull(val timeInMillis: Long,
                         val tz: MetroTimeZone
): Parcelable, Comparable<TimestampFull>, Timestamp() {
    ...
    override fun format(): String = TimestampFormatter.dateTimeFormat(this)
