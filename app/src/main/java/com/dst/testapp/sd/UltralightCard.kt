    ...
    override val manufacturingInfo: List<ListItemInterface>?
        get() {
            ...
            if (cardModel.isNotEmpty()) {
                ret += ListItem("Ultralight model",cardModel)
                try {
                    val modelEnum = UltralightType.valueOf(cardModel)
                    ret += ListItem(
                        "Size",
                        "R.plurals.mfu_num_pages, ${modelEnum.pageCount} ${modelEnum.pageCount}"
                    )
            ...
            if (cardRawModel?.versionCmd != null) {
                ...
                ret += listOf(
                    HeaderListItem("Get version response"),
                    ListItem("Raw response", vcmd.toHexDump()),
                    ListItem(
                        "Vendor", NumberUtils.mapLookupLocalize(vcmd[1].toInt() and 0xff, mapOf(4 to "NXP Semiconductors"))
                    ),
                    ListItem(
                        "Product type", NumberUtils.mapLookup(vcmd[2].toInt() and 0xff, mapOf(3 to "Ultralight", 4 to "NTAG")
                        )
                    ),
                    ListItem(
                        "Product subtype", NumberUtils.mapLookup(vcmd[3].toInt() and 0xff, mapOf(1 to "17 pF"))
                    ),
                    ListItem(
                        "Version", NumberUtils.zeroPad(vcmd[4].toString(16), 2) + "." + NumberUtils.zeroPad(vcmd[5].toString(16), 2)
                    ),
                    ListItem(
                        "Size ID", NumberUtils.zeroPad(vcmd[6].hexString, 2)
                    ),
                    ListItem(
                        "Protocol type", NumberUtils.mapLookup(
